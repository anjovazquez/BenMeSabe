 ...
 */
@PerActivity
public class ProductMenuListPresenter extends DefaultSubscriber<List<Product>> implements Presenter {

    private UseCase getProductListUseCase;
    private ProductListView viewListView;

    @Inject
    public ProductMenuListPresenter(@Named("productList") UseCase getProductListUserCase) {
    ...
    private final class ProductListSubscriber extends DefaultSubscriber<List<Product>> {
        ...
        @Override public void onCompleted() {
            ProductMenuListPresenter.this.hideViewLoading();
        }

        @Override public void onError(Throwable e) {
            ProductMenuListPresenter.this.hideViewLoading();
            ((Exception) e).printStackTrace();
            ProductMenuListPresenter.this.showErrorMessage(new DefaultErrorBundle((Exception) e));
            ProductMenuListPresenter.this.showViewRetry();
        }

        @Override public void onNext(List<Product> products) {
            ProductMenuListPresenter.this.showProductCollectionInView(products);
        }
    }
}
